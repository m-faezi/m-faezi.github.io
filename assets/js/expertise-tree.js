...
class FloatingClock {
    ...
    init() {
        ...
        this.container.id = 'floatingClock';
        this.container.innerHTML = `
            <div class="clock">
                ...
                ${Array.from({length: 12}, (_, i) =>
                    `<div class="hour-mark" style="transform: rotate(${i * 30}deg)"></div>`
                ).join('')}
            </div>
        `;
    ...
    updateClock() {
        ...
        const clock = this.container.querySelector('.clock');
...
// Update your initialization to include the clock
document.addEventListener('DOMContentLoaded', function() {
    console.log('DOM loaded - starting all components...');
...
setTimeout(function() {
    ...
    // Initialize clock if not already done
    if (!document.getElementById('floatingClock')) {
        new FloatingClock();
    }
}, 3000);
